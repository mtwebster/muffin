                      ...
                      GError         **error)
{
  ...
  new_sync = egl->eglCreateSync (display, type, attrib_list);
  if (new_sync == EGL_NO_SYNC)
    {
      set_egl_error (error);
      return FALSE;
    }
...
/* Queues a wait on the GPU rather than blocking the CPU: subsequent commands
 * on this display will not execute until the sync is signalled. */
gboolean
