...
meta_wayland_layer_shell_sync_keyboard_focus (MetaWaylandCompositor *compositor)
{
  ...
  /* Keyboard focus is gone or stuck on a defunct layer surface. If the
   * display still has a focus window, re-attach the keyboard to it - it may
   * have been focused while an exclusive surface held the keyboard, in which
   * case it never received it (and re-clicking it won't change focus_window,
   * so nothing else will re-send keyboard focus). Otherwise fall back to the
   * MRU window. */
